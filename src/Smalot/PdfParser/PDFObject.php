            ...
            foreach($commands as $command) {
                ...
                switch($command[self::OPERATOR]) {
                    ...
                    case 'Td':
                        ...
                        else if($current_position_td['x'] !== false && floatval($x) > floatval(
        ...
        while($offset < strlen($text_part)) {
            ...
            switch($char) {
                ...
                default:
                    ...
                    else if(preg_match('/^\s*([0-9\.\-]+\s*?)+\s*/si', substr($text_part, $offset), $matches)) {
                        $type = 'n';
                        $command = trim($matches[0]);
                        $offset += strlen($matches[0]);
                    }
                    else if(preg_match('/^\s*([A-Z\*]+)\s*/si', substr($text_part, $offset), $matches)) {
                        $type = '';
                        $operator = $matches[1];
                        $command = '';
                        $offset += strlen($matches[0]);
